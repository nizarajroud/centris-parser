...
def main(user_data_dir: str = None, headless: bool = None) -> None:
    ...
    with NovaAct(
        ...
        # nova.act("Click on the first home listing")
        ...
        def extract_field(field_name):
            ...
            if label:
                ...
                if parent:
                    ...
                    next_div = label.find_parent("div").find_next_sibling("div")
                    if next_div:
                        value = next_div.find("span", class_="d-fontSize--smallest")
                        return value.get_text(strip=True) if value else ""
            return ""
        
        # Extract Centris No. and Date Sent
        ...
        fields = {
            ...
            "Occupancy": extract_field("Occupancy"),
            "Expected Delivery Date": extract_field("Expected Delivery Date"),
            "Deed of Sale Signature": extract_field("Deed of Sale Signature"),
            "Building Size": extract_field("Building Size"),
            "Lot Eval.": extract_field("Lot Eval."),
            "Living Area": extract_field("Living Area"),
            "Building Eval.": extract_field("Building Eval."),
            "Lot Size": extract_field("Lot Size"),
            "Mun. Taxes": extract_field("Mun. Taxes"),
            "Lot Area": extract_field("Lot Area"),
            "School Taxes": extract_field("School Taxes"),
            "Cert. of Location": extract_field("Cert. of Location"),
            "Additional Rev.": extract_field("Additional Rev."),
            "Body of Water": extract_field("Body of Water"),
            "Intergenerational": extract_field("Intergenerational"),
            "Seasonal": extract_field("Seasonal"),
            "No. of Rooms": extract_field("No. of Rooms"),
        }
        
        # with open("first_home_details.txt", "w", encoding="utf-8") as f:
        with open("second_home_details.txt", "w", encoding="utf-8") as f:
